...
int main()
{
	...
	assert(mmap((void *)(start - len), len + 1, prot, MAP_ANONYMOUS, -1) ==
	       -1);
	assert(mmap((void *)(start + len + 1), len, prot, MAP_ANONYMOUS, -1) ==
	       -1);
	assert(mmap((void *)(start + len), len, 0, MAP_ANONYMOUS, -1) == -1);
	assert(mmap((void *)(start + len), len, prot | 8, MAP_ANONYMOUS, -1) ==
	       -1);
