...
int main()
{
	...
	if (pid, 0) {
		...
		assert_eq(mailread(buffer, BUF_LEN), -1);
		...
		for (int i = 0; i < 16; ++i) {
			char buffer[BUF_LEN];
			memset(buffer, 0, sizeof(buffer));
			assert_eq(mailread(buffer, BUF_LEN), BUF_LEN);
			char ans[BUF_LEN];
			memset(ans, i, BUF_LEN);
			assert_eq(strncmp(ans, buffer, BUF_LEN), 0);
		}
		...
		assert_eq(strncmp(buffer, ans, BUF_LEN), 0);
	...
	for (int i = 0; i < 16; ++i) {
		...
		assert_eq(mailwrite(pid, buffer, BUF_LEN), BUF_LEN);
	}
	puts("father write 16 mails succeed");
	char buffer[BUF_LEN];
	memset(buffer, 16, BUF_LEN);
	assert_eq(mailwrite(pid, buffer, BUF_LEN), -1);
